...
class WebSocketChannel {
  ...
  once (eventName, listener) {
    if (this.name == null && WebSocketChannel.NO_WRAP_EVENTS.indexOf(eventName) >= 0) { this._emitter.once(eventName, listener, this) } else { this._emitter.once(eventName, this._wrapListener(listener)) }
    return this
  }

  removeListener (eventName, listener) {
    if (this.name == null && WebSocketChannel.NO_WRAP_EVENTS.indexOf(eventName) >= 0) { this._emitter.removeListener(eventName, listener) } else {
  ...
  listeners (eventName) {
    if (this.name == null && WebSocketChannel.NO_WRAP_EVENTS.indexOf(eventName) >= 0) { return this._emitter.listeners(eventName) } else {
  ...
  request (eventName) {
    ...
    var ret = this._wrapper._sendEvent(this.name, eventName, arguments, true)
